...
function bindStaticEvents() {
    ...
    if (missing.length) {
        throw new Error(
            `No se pudieron enlazar los eventos iniciales. Faltan: ${missing.join(", ")}.`,
        );
    }
    ...
    dom.downloadButton.addEventListener("click", exportAgendaPdf);
}
...
async function bootstrap() {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (!user) {
            clearSessionState();
            showAuthScreen(
                "Escriba la congregación y su número para abrir la agenda.",
            );
            return;
        }
...
async function handleCongregationLogin(event) {
    ...
    try {
        const loginWithCongregationCode = httpsCallable(
            appState.functions,
            LOGIN_FUNCTION_NAME,
        );
        ...
        showAuthScreen(
            normalizeFirebaseError(
                error,
                "No se pudo validar la congregación y el número.",
            ),
        );
    } finally {
        setAuthBusy(false);
...
async function loadAgendaForCongregation(user) {
    ...
    try {
        ...
        if (!congregationId) {
            throw new Error(
                "La sesión actual no tiene `congregation_id` en el token.",
            );
        }
        ...
        if (!activity) {
            throw new Error(
                "No hay visitas de congregación disponibles para esta congregación.",
            );
        }

        const agendaDoc = await ensureAgendaDocument(
            activity,
            congregationDoc?.name || congregationName,
        );
        const mergedAgenda = mergeAgendaWithDefaults(
            agendaDoc.agendaData,
            buildDefaultAgenda(activity, congregationDoc),
        );
...
function loadAdminPreviewMode() {
    ...
    appState.agenda.meetings[0].place = "Salón del Reino";
    appState.agenda.meetings[1].time = "10:00";
    appState.agenda.meetings[1].place = "Salón del Reino";
    ...
    appState.agenda.lunches[0].time = "12:00";
...
async function fetchTargetActivity(congregationId) {
    ...
    const activities = snapshot.docs
        ...
        .sort(
            (left, right) => toDate(left.week_start) - toDate(right.week_start),
        );
    ...
    const nextVisit = activities.find(
        (entry) => startOfDay(toDate(entry.week_start)) >= today,
    );
...
async function ensureAgendaDocument(activity, congregationName) {
    ...
    const newDoc = {
        ...
        congregationName: (congregationName || activity.congregationName || "")
            .split("-")[0]
            .trim(),
...
function buildDefaultAgenda(activity = null, congregation = null) {
    const rawCongregationName =
        congregation?.name || activity?.congregationName || "";
    const congregationName = rawCongregationName.split("-")[0].trim();
    const visitDate = activity?.week_start
        ? formatDateInput(activity.week_start)
        : "";
...
function mergeAgendaWithDefaults(saved, defaults) {
    ...
    return {
        congregation: saved.congregation
            ? saved.congregation.split("-")[0].trim()
            : defaults.congregation,
        date: saved.date ?? defaults.date,
        meetings: defaults.meetings.map((item, index) => ({
            ...item,
            ...(saved.meetings?.[index] || {}),
            place: saved.meetings?.[index]?.place || item.place,
        })),
        fieldService: defaults.fieldService.map((item, index) => ({
            ...item,
            ...(saved.fieldService?.[index] || {}),
        })),
        henry: defaults.henry.map((item, index) => ({
            ...item,
            ...(saved.henry?.[index] || {}),
        })),
        carolina: defaults.carolina.map((item, index) => ({
            ...item,
            ...(saved.carolina?.[index] || {}),
        })),
        lunches: defaults.lunches.map((item, index) => ({
            ...item,
            ...(saved.lunches?.[index] || {}),
        })),
    };
...
function renderAgenda() {
    const displayName = appState.congregationName
        ? appState.congregationName.split("-")[0].trim()
...
function renderMeetings() {
    ...
    appState.agenda.meetings.forEach((meeting, index) => {
        ...
        timeInput.addEventListener("input", (e) =>
            updateNested("meetings", index, "time", e.target.value),
        );
        daySelect.addEventListener("change", (e) =>
            updateNested("meetings", index, "day", e.target.value),
        );
        placeInput.addEventListener("input", (e) =>
            updateNested("meetings", index, "place", e.target.value),
        );
...
function renderFieldService() {
    ...
    appState.agenda.fieldService.forEach((entry, index) => {
        ...
        tagEl.textContent =
            entry.pm === "No programar" || entry.day === "Jueves"
                ? "PM libre"
                : "Día activo";
        ...
        amPlaceInput.addEventListener("input", (e) =>
            updateNested("fieldService", index, "amPlace", e.target.value),
        );
        ...
        pmPlaceInput.addEventListener("input", (e) =>
            updateNested("fieldService", index, "pmPlace", e.target.value),
        );
...
function renderHenry() {
    ...
    appState.agenda.henry.forEach((entry, index) => {
        ...
        timeInput.addEventListener("input", (e) =>
            updateNested("henry", index, "time", e.target.value),
        );
        ...
        body.append(
            ...
            makePersonField(
                "Lección o información",
                "textarea",
                entry.notes,
                "Anote detalles importantes",
...
function renderCarolina() {
    ...
    const entriesWithIndex = appState.agenda.carolina.map((entry, index) => ({
        entry,
        index,
    }));
    const dayOrder = [
    ...
    entriesWithIndex.sort(
        (a, b) => dayOrder.indexOf(a.entry.day) - dayOrder.indexOf(b.entry.day),
    );
    ...
    entriesWithIndex.forEach(({ entry, index }) => {
        ...
        if (isVisible) {
            ...
            timeInput.value = entry.time || "";
            timeInput.addEventListener("input", (e) =>
                updateNested("carolina", index, "time", e.target.value),
            );
            timePill.append(timeInput);
...
function renderLunches() {
    ...
    appState.agenda.lunches.forEach((entry, index) => {
        ...
        familyInput.addEventListener("input", (e) =>
            updateNested("lunches", index, "family", e.target.value),
        );
        addressInput.addEventListener("input", (e) =>
            updateNested("lunches", index, "contact", e.target.value),
        );
        notesInput.addEventListener("input", (e) =>
            updateNested("lunches", index, "note", e.target.value),
        );
...
function createField({
    ...
    input.addEventListener(type === "select" ? "change" : "input", (event) =>
...
function makePersonField(label, type, value, placeholder, extraClass, onInput, options = []) {
    ...
    input.addEventListener(type === "select" ? "change" : "input", (e) => onInput(e.target.value));
...
function queueSave() {
    ...
    saveTimeout = window.setTimeout(async () => {
        try {
            ...
            showAgendaMessage(
                normalizeFirebaseError(error, "No se pudo guardar la agenda."),
            );
        }
    }, 300);
}
...
function renderSummary() {
    ...
    dom.summaryCongregation.textContent =
        summaryName.split("-")[0].trim() || "Sin definir";
...
function countProgress(agenda) {
    const values = [
        agenda.congregation,
        agenda.date,
        ...agenda.meetings.flatMap((entry) => [
            entry.day,
            entry.time,
            entry.place,
        ]),
        ...agenda.fieldService.flatMap((entry) => [
            entry.am,
            entry.amPlace,
            entry.pm,
            entry.pmPlace,
        ]),
        ...agenda.henry.flatMap((entry) => [
            entry.time,
            entry.type,
            entry.companion,
            entry.notes,
        ]),
        ...agenda.carolina.flatMap((entry) => [
            entry.time,
            entry.sister,
            entry.lesson,
        ]),
        ...agenda.lunches.flatMap((entry) => [
            entry.time,
            entry.family,
            entry.contact,
            entry.note,
...
async function handleResetAgenda() {
    ...
    if (
        !window.confirm(
            "Se restaurará la agenda base de esta visita. ¿Desea continuar?",
        )
    ) {
        return;
    }
...
function exportAgendaPdf() {
    ...
    if (!printWindow) {
        showAgendaMessage(
            "No se pudo abrir la ventana de impresión. Permita ventanas emergentes para descargar el PDF.",
        );
        return;
    }

    printWindow.document.write(buildPrintableAgendaDocument(appState.agenda));
...
function buildPrintableAgendaDocument(agenda) {
    const title = `Agenda ${agenda.congregation || "visita"}`;
...
<html lang="es">
  <head>
    ...
    <style>${getPrintableAgendaStyles()}</style>
  </head>
  <body>
    <main class="print-agenda">
      <header class="print-agenda__header">
        <p>Agenda para la visita del superintendente viajante</p>
        <h1>${escapeHtml(agenda.congregation || "Congregación")}</h1>
        <strong>${escapeHtml(formatDisplayDate(agenda.date))}</strong>
      </header>
      ${renderPrintMeetings(agenda.meetings)}
      ${renderPrintFieldService(agenda.fieldService)}
      ${renderPrintVisits("Estudios, revisitas y pastoreos con Henry Vélez", agenda.henry, "companion", "notes")}
      ${renderPrintVisits("Estudios y revisitas con Carolina Vélez", agenda.carolina, "sister", "lesson")}
      ${renderPrintLunches(agenda.lunches)}
    </main>
  </body>
</html>`;
}

function renderPrintMeetings(meetings) {
...
function renderPrintFieldService(entries) {
    return renderPrintSection(
        "Servicio del campo",
        ["Día", "AM", "Lugar AM", "PM", "Lugar PM"],
        entries.map((entry) => [
            entry.day,
            formatTime(entry.am),
            entry.amPlace,
            entry.pm === "No programar" ? "No programar" : formatTime(entry.pm),
            entry.pmPlace,
        ]),
    );
}

function renderPrintVisits(title, entries, personKey, noteKey) {
    return renderPrintSection(
        title,
        ["Día", "Hora", "Persona", "Información"],
        entries
            .filter((entry) => entry.time || entry[personKey] || entry[noteKey])
            .map((entry) => [
                entry.day,
                formatTime(entry.time),
                entry[personKey],
...
function renderPrintLunches(entries) {
    return renderPrintSection(
        "Arreglos para almuerzos",
        ["Día", "Hora", "Familia", "Dirección / teléfono"],
        entries.map((entry) => [
            entry.day,
            formatTime(entry.time || "12:00"),
            entry.family,
            [entry.contact, entry.note].filter(Boolean).join(" · "),
        ]),
    );
}

function renderPrintSection(title, headings, rows) {
...
function getPrintableAgendaStyles() {
    return `
      ...
      * { box-sizing: border-box; }
      ...
      .print-agenda__header {
        border-top: 5px solid #202728;
        padding: 6px 0 8px;
        display: grid;
      ...
      .print-agenda__header h1 {
        margin: 0;
        font-size: 18px;
        line-height: 1;
        text-transform: uppercase;
      }
      .print-agenda__header strong {
        font-size: 10px;
        white-space: nowrap;
      }
      .print-section {
...
function getMeetingOptions(title) {
    return (
        MEETING_DEFINITIONS.find((entry) => entry.title === title)?.options ||
        WEEKDAY_OPTIONS
    );
}
...
function normalizeFirebaseError(error, fallback) {
    ...
    if (
        ...
        message.includes("auth/custom-token-mismatch")
    ) {
    ...
    if (
        ...
        message.includes("auth/invalid-custom-token")
    ) {
